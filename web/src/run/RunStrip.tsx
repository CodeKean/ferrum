...
function RunAnnouncer({ run }: { run: RunState }) {
  const accounted = run.done + run.errors + run.skipped;
  const pct = run.total > 0 ? Math.min(100, Math.round((accounted / run.total) * 10) * 10) : 0;

  const message =
    run.status === "done" ? `Run finished. ${run.done.toLocaleString()} done, ${run.errors.toLocaleString()} failed.`
  ...
  if (finished) {
    ...
    return (
      <>
        ...
        <span className="cc-rs__seg cc-rs__count mono">{run.done.toLocaleString()} done</span>
        ...
        <span
          ...
          title={run.costUsd > 0 ? "Estimated from list prices — not a bill." : "Nothing in this run bills anything."}
        >
...
function ProgressBar({ run, pct }: { run: RunState; pct: number }) {
  ...
  return (
    ...
    <span
      ...
      aria-valuemin={0}
      aria-valuemax={100}
      aria-valuetext={`${pct}% — ${(run.done + run.errors + run.skipped).toLocaleString()} of ${run.total.toLocaleString()} rows`}
    >
      <span className="cc-rs__fill cc-rs__fill--done" style={{ width: w(run.done) }} />
      <span className="cc-rs__fill cc-rs__fill--skip" style={{ width: w(run.done + run.skipped), zIndex: 1 }} />
      <span className="cc-rs__fill cc-rs__fill--err" style={{ width: w(run.done + run.skipped + run.errors), zIndex: 0 }} />
    </span>
  );
}
